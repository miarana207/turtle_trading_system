...
def screen_spot(
    ...
    quote_assets: Optional[set[str]] = None,
    warnings: Optional[List[str]] = None,
) -> Tuple[List[Dict[str, Any]], CriterionAudit]:
    """Applique uniquement les filtres structurels/liquidité Spot.

    Aucun indicateur technique n'est calculé. Le classement qualité est effectué
    plus tard, dans la partie reporting, sur les survivants uniquement.
    """
    ...
    assets = apply_criterion(
        ...
        lambda x: str(x.get("quoteAsset", "")).upper() in effective_quotes,
    )
    ...
    # Diagnostic de distribution du volume AVANT le filtre de volume.
    ...
    audit.diagnostics["volume_distribution"] = {
        threshold: sum(
            1 for a in volume_universe if as_float(a.get("quoteVolume")) >= threshold
        )
        for threshold in volume_thresholds
    }
    audit.diagnostics["volume_classes"] = {
        "< $250k": sum(1 for a in volume_universe if as_float(a.get("quoteVolume")) < 250_000),
        "$250k – $500k": sum(1 for a in volume_universe if 250_000 <= as_float(a.get("quoteVolume")) < 500_000),
        "$500k – $1M": sum(1 for a in volume_universe if 500_000 <= as_float(a.get("quoteVolume")) < 1_000_000),
        "$1M – $2M": sum(1 for a in volume_universe if 1_000_000 <= as_float(a.get("quoteVolume")) < 2_000_000),
        "$2M – $5M": sum(1 for a in volume_universe if 2_000_000 <= as_float(a.get("quoteVolume")) < 5_000_000),
        "$5M – $10M": sum(1 for a in volume_universe if 5_000_000 <= as_float(a.get("quoteVolume")) < 10_000_000),
        "$10M – $20M": sum(1 for a in volume_universe if 10_000_000 <= as_float(a.get("quoteVolume")) < 20_000_000),
        "$20M – $50M": sum(1 for a in volume_universe if 20_000_000 <= as_float(a.get("quoteVolume")) < 50_000_000),
        "$50M – $100M": sum(1 for a in volume_universe if 50_000_000 <= as_float(a.get("quoteVolume")) < 100_000_000),
        "$100M – $500M": sum(1 for a in volume_universe if 100_000_000 <= as_float(a.get("quoteVolume")) < 500_000_000),
        "$500M – $1B": sum(1 for a in volume_universe if 500_000_000 <= as_float(a.get("quoteVolume")) < 1_000_000_000),
        ">= $1B": sum(1 for a in volume_universe if as_float(a.get("quoteVolume")) >= 1_000_000_000),
    }
    ...
    if ORDER_BOOK_DEPTH_ENABLED:
        ...
        # Mesure Depth/Volume avant le filtre de profondeur, conformément au
        # cahier des charges. Aucune décision supplémentaire n'est prise sur le ratio.
        depth_ratios: List[float] = []
        for asset in assets:
            volume = as_float(asset.get("quoteVolume"))
            depth = as_float(asset.get("depthTotalQuote"))
            ratio = (depth / volume * 100.0) if volume > 0 else None
            asset["depthToVolumePercent"] = ratio
            asset["depthVolumeStatus"] = depth_volume_status(asset)
            if ratio is not None and math.isfinite(ratio):
                depth_ratios.append(ratio)

        classes = {
            "< 0.1%": sum(1 for v in depth_ratios if v < 0.1),
            "0.1% – 0.5%": sum(1 for v in depth_ratios if 0.1 <= v < 0.5),
            "0.5% – 1%": sum(1 for v in depth_ratios if 0.5 <= v < 1.0),
            "1% – 2%": sum(1 for v in depth_ratios if 1.0 <= v < 2.0),
            "2% – 5%": sum(1 for v in depth_ratios if 2.0 <= v < 5.0),
            "5% – 10%": sum(1 for v in depth_ratios if 5.0 <= v < 10.0),
            "10% – 25%": sum(1 for v in depth_ratios if 10.0 <= v < 25.0),
            "25% – 50%": sum(1 for v in depth_ratios if 25.0 <= v < 50.0),
            "50% – 100%": sum(1 for v in depth_ratios if 50.0 <= v < 100.0),
            ">= 100%": sum(1 for v in depth_ratios if v >= 100.0),
        }
        audit.diagnostics["depth_volume"] = {
            "count": len(depth_ratios),
            "minimum": min(depth_ratios) if depth_ratios else None,
            "p25": percentile(depth_ratios, 0.25),
            "median": percentile(depth_ratios, 0.50),
            "mean": (sum(depth_ratios) / len(depth_ratios)) if depth_ratios else None,
            "p75": percentile(depth_ratios, 0.75),
            "maximum": max(depth_ratios) if depth_ratios else None,
            "classes": classes,
        }
        ...
        for asset in assets:
            asset["depthToVolumePercent"] = None
            asset["depthVolumeStatus"] = "DÉSACTIVÉE"
    ...
    required_history_bars = max(1, MIN_HISTORY_DAYS * 24)

    def closed_klines_only(klines: List[List[Any]]) -> List[List[Any]]:
        """Conserve uniquement les bougies 1H dont le close time est déjà passé."""
        ...
        for candle in klines:
            ...
            if close_time_ms <= now_ms:
                closed.append(list(candle))
        return closed

    def has_minimum_history(asset: Dict[str, Any]) -> bool:
        try:
            # Demande une bougie supplémentaire afin de pouvoir éliminer
            # la bougie 1H éventuellement encore en formation.
            klines = spot_klines(asset["symbol"], "1h", required_history_bars + 1)
            closed = closed_klines_only(klines)
            asset["historyBars"] = len(closed)
            asset["historyDays"] = len(closed) / 24.0
            return len(closed) >= required_history_bars
        except Exception as exc:
